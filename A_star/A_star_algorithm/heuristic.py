...
def So_O_Dat_Sai_Cho(current,goal):
    ...
    for i in range(len(goal)):
        for j in range(len(goal[i])):
            if current[i][j] != goal[i][j] and current[i][j]!=0: count += 1
    return count
...
# print(So_O_Dat_Sai_Cho(origin,goal))
def Edge_Matching_Heuristic(current, goal):
    ...
    for i in range(len(current)):
        for j in range(len(current[i])):
            ...
            target_pos = [(index, row.index(current[i][j])) for index, row in enumerate(goal) if current[i][j] in row][
                0]
